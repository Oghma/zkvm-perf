...
use reth_primitives::{
    Address, Bloom, BufMut, Header, Receipt, ReceiptWithBloom, Transaction, TransactionSigned, U256,
};
use reth_trie::{root::ordered_trie_root_with_encoder, TrieAccount};
use revm::db::{AccountState, InMemoryDB};
use revm::{
    primitives::{Account, SpecId, TransactTo, TxEnv},
    ...
    <D as Database>::Error: core::fmt::Debug,
{
    ...
    pub fn execute(&mut self) {
        ...
        for (tx_no, tx) in self.input.transactions.iter().enumerate() {
            ...
            // Setup EVM from tx.
            fill_eth_tx_env(evm.tx_mut(), &tx.transaction, tx_from);
            // Execute transaction.
            let res = evm
                .transact()
                .map_err(|e| {
